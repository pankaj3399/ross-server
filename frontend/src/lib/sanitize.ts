...
/**
 * Sanitize AIMA question/domain descriptions while allowing a safe set of formatting tags
 * @param html - The HTML content to sanitize
 * @returns Sanitized HTML string
 */
export function sanitizeAimaDescription(html: string): string {
  if (!html || typeof html !== "string") {
    return "";
  }

  // Configure DOMPurify to allow only specific safe tags for AIMA descriptions
  // We allow basic formatting and list tags that are generated by formatAimaDescription
  // and might be present in manual DB entries.
  return DOMPurify.sanitize(html, {
    ALLOWED_TAGS: [
      "p", "br", "strong", "b", "em", "i", "u", "ul", "ol", "li", "span", "div"
    ],
    ALLOWED_ATTR: ["class", "style"], // Allow styling classes for lists
    RETURN_TRUSTED_TYPE: false, // Standard string return
  }) as string;
}
